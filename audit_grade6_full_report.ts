...
async function auditGrade6Full() {
  ...
  console.log(`Bắt đầu quét dữ liệu Khối Lớp 6... Tìm thấy ${chapters.length} chương.\n`);

  for (const chapter of chapters) {
    console.log(`**Khối:** Lớp 6\n**Block rà soát:** ${chapter.name}`);
    ...
    for (const subCat of subCategories) {
      ...
      if (questions.length === 0) {
          ...
          hasAnyError = true;
          totalErrors++;
          report += `| Lớp 6 | ${chapter.name} | ${subCat.name} | Thiếu toàn bộ câu hỏi | Cần chèn thêm dữ liệu | Chờ sửa |\n`;
      }
      ...
      for (const q of questions) {
        ...
        if (errors.length > 0) {
          ...
          hasAnyError = true;
          totalErrors++;
          report += `| Lớp 6 | ${chapter.name} | ${subCat.name} (Mã: ${q.question_code}) | ${errors.join(', ')} | Cần bổ sung/cập nhật dữ liệu | Chờ sửa |\n`;
        }
      }
  ...
  if (hasAnyError) {
    ...
    console.log('\n✅ **TẤT CẢ CÁC CHƯƠNG LỚP 6 ĐÃ ĐẠT CHUẨN 100%**');
  }
}

auditGrade6Full().catch(console.error);
