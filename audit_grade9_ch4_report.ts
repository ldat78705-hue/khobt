...
async function auditGrade9Chapter4() {
  const chapter4Id = '41ec0379-b88d-4db0-903d-bf2ac168af9a';
  
  const categories = await sql`
    SELECT id, name, parent_id, sort_order
    FROM public.categories
    WHERE (parent_id = ${chapter4Id} OR id = ${chapter4Id})
    AND grade = 9
    ORDER BY sort_order
  `;
  
  const subCategories = categories.filter((c: any) => c.parent_id === chapter4Id);
  ...
  for (const subCat of subCategories) {
    ...
    for (const q of questions) {
      ...
      if (errors.length > 0) {
        hasError = true;
        totalErrors++;
        report += `| Lớp 9 | Chương IV | ${subCat.name} (Mã: ${q.question_code}) | ${errors.join(', ')} | Cần bổ sung/cập nhật dữ liệu | Chờ sửa |\n`;
      }
    }
  ...
  if (totalErrors > 0) {
    ...
    console.log('\n✅ TOÀN BỘ CHƯƠNG IV - ĐÃ ĐẠT - CHÍNH XÁC 100%');
  }
}

auditGrade9Chapter4().catch(console.error);
