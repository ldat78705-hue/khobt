...
async function auditGrade7Full() {
  ...
  console.log(`Bắt đầu quét dữ liệu Khối Lớp 7... Tìm thấy ${chapters.length} chương.\n`);

  for (const chapter of chapters) {
    console.log(`**Khối:** Lớp 7\n**Block rà soát:** ${chapter.name}`);
    ...
    for (const subCat of subCategories) {
      ...
      if (questions.length === 0) {
          ...
          hasAnyError = true;
          totalErrors++;
          report += `| Lớp 7 | ${chapter.name} | ${subCat.name} | Thiếu toàn bộ câu hỏi | Cần chèn thêm dữ liệu | Chờ sửa |\n`;
      }
      ...
      for (const q of questions) {
        ...
        if (errors.length > 0) {
          ...
          hasAnyError = true;
          totalErrors++;
          report += `| Lớp 7 | ${chapter.name} | ${subCat.name} (Mã: ${q.question_code}) | ${errors.join(', ')} | Cần bổ sung/cập nhật dữ liệu | Chờ sửa |\n`;
        }
      }
  ...
  if (hasAnyError) {
    ...
    console.log('\n✅ **TẤT CẢ CÁC CHƯƠNG LỚP 7 ĐÃ ĐẠT CHUẨN 100%**');
  }
}

auditGrade7Full().catch(console.error);
