...
function normalizeLatexAndText(text) {
  ...
  // Fix basic spacing around operators in text (be careful not to break latex)
  // We skip this to avoid breaking complex latex, but we can do some safe ones:
  newText = newText.replace(/([a-zA-Z0-9])\+([a-zA-Z0-9])/g, '$1 + $2');
...
async function main() {
  console.log('Fetching questions for Grade 5 and 6...');
  const questions = await sql`SELECT id, content, answer, solution FROM public.questions WHERE grade IN (5, 6)`;
  ...
  console.log(`QA completed! Total questions updated: ${updatedCount} / ${questions.length}`);
}
